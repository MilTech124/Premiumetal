       ...
       <img src="/PREMIUM.jpg" alt="logo" className='w-[80px] max-sm:absolute left-0 max-sm:w-[50px] ' />
        <nav className='md:ml-auto md:pr-4 md:!flex'style={{ display: isOpen ? "flex" : "none" }}>
            <ul className='md:flex  max-sm:flex-col max-sm:flex max-sm:gap-2 items-center  gap-10 font-semibold max-md:justify-center'>
                ...
                <li onClick={()=>{setIsOpen(false)}}> <Link href="https://backend.premiumetal.pl/backed/konfigurator-sk/" >Garážový konfigurátor</Link></li>
        ...
        <div className='socials top-3 max-sm:left-6 md:right-6 md max-sm:py-2 md:pr-5 md:!flex' style={{ display: isOpen ? "flex" : "none" }}>
            <Link href='https://www.facebook.com/profile.php?id=61557453681909'><img className='hover:cursor-pointer w-10 hover:scale-110' src="/facebook.png" alt="facebook" /></Link>  
        </div>
